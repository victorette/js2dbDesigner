Ext.define('JS2DBDesigner.controller.DiagramMenuController', {
    ...
    getAllTablesFromDB: function() {
        ...
        Ext.Ajax.request({
            url: DBDesigner._PConfig.urlGetEntitiesJSONDiagram,
            params: {
                projectID: projectID
            },
            success: function(response) {
                ...
                var store = Ext.create('JS2DBDesigner.store.DiagramModelStore', {
                    data: outcome.tables
                });
				...
				searchGrid.getView().getRowClass = function(record, rowIndex, rowParams, store) {
					controller = JS2DBDesigner.app.getController('DiagramController');
	...
	syncDiagramFromDB: function() {
		...
		function doUpdate(btn) {
            if (btn === 'yes') {
                Ext.Ajax.request({
					url: DBDesigner._PConfig.synchDiagramFromDB,
					params: {
						projectID: projectID
					},
					success: function(response) {
						...
						Ext.Msg.alert('Success', DBDesigner.__language.Message_Diagram_Synchronized);
					},
					failure: function(response) {
						Ext.Msg.alert('Failure', DBDesigner.__language.Message_Error_Diagram_Synchronized);
					}
				});
            }
        }
		Ext.MessageBox.confirm('Confirmation', DBDesigner.__language.Msg_Confirm_Delete_Operation + ' les configurations personnalisées seront perdues!', doUpdate);
    ...
    menuManageDiagram: function() {
        ...
        var win = Ext.create('Ext.window.Window', {
            width: 500,
            title: DBDesigner.__language.Title_Diagrams,
            layout: 'fit',
            items: {
                xtype: 'diagramgrid',
            }
    ...
    addTableFromSearchGrid: function(button, e, eOpts) {
        ...
        diagramController.showProgressBar(DBDesigner.__language.Message_Add_Table_Canvas,'Processing...');
        ...
        Ext.Ajax.request({
            url: DBDesigner._PConfig.getElementsDiagramFromSelectedTables,
    ...
    editDiagram: function(grid, record) {
        var edit = Ext.create('JS2DBDesigner.view.diagram.base.Form').show();
    ...
    updateDiagram: function(button) {
        ...
        if (form.getForm().isValid()) {
            if (values.id > 0) {
                ...
                record = Ext.create('JS2DBDesigner.model.Diagram');
    ...
    deleteDiagram: function(button) {
		...
		if (openedDiagramID !== record[0].data.id){
			...
			Ext.Msg.alert('Warning', DBDesigner.__language.Warning_Delete_Active_Diagram);
		}
    },
    ...
    openSelectedDiagram: function(button, event) {
		...
		if (openedDiagramID !== diagramID){
	        diagramController.runAjaxOpenDiagram(controller, DBDesigner._PConfig.openDiagram, projectID, diagramID);
		} else {
			Ext.Msg.alert('Warning', DBDesigner.__language.Warning_Diagram_Already_Open);
		}
    },
