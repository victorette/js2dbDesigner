Ext.define('JS2DBDesigner.controller.DiagramContextMenuController', {
    ...
    addInputPort: function(button, e, eOpts) {
        ...
        var positions = Ext.create('JS2DBDesigner.store.PortPositions');
        var options = {
            label: 'Position du port',
            store: positions,
            userData: {controller: diagramController, table: table}
        };
        DBDesigner.ComboBoxPrompt.prompt('Nouveau port', options, function(btn, text, cfg) {
            if (btn == 'ok') {
