 ...
 */
Ext.define('JS2DBDesigner.controller.DiagramController', {
    extend: 'Ext.app.Controller',
    ...
    createEntityAttribute: function(text, id, pk, fk, datatype) {
        return Ext.create('JS2DBDesigner.model.EntityAttributesModel', {
            text: text,
            id: id,
            inputPort: '',
            datatype: 'string',
    ...
    saveDiagram: function(button, e, eOpts) {
        this.showProgressBar(DBDesigner.__language.Message_Saving_Data, DBDesigner.__language.Text_Submit_Validation_Form);
        ...
        this.createAjaxRequest(DBDesigner._PConfig.saveDiagram, "POST", params, Ext.JSON.encode(jsonDocument), successFunction, failureFunction);
    ...
    synchDBFromDiagram: function(button, e, eOpts) {
        var controller = this;
        controller.showProgressBar(DBDesigner.__language.Message_Creating_Objects, DBDesigner.__language.Text_Submit_Validation_Form);
        params = {
            projectID: controller.getDiagramMainView().getProjectID()
        };
        successFunction = function(response) {
        ...
        this.createAjaxRequest(DBDesigner._PConfig.synchDBFromDiagram, "POST", params, jsonDocument, successFunction, failureFunction);
    ...
    runAjaxOpenDiagram: function(controller, url, projectID, diagramID) {
        Ext.Ajax.request({
            ...
            success: function(response) {
                ...
                //controller.getDiagramMainView().setTitle(DBDesigner.__language.Title_Work_Diagram + outcome.diagramCode);
    ...
    openDiagram: function(button, e, eOpts) {
        ...
        if (diagramID === null) {
            controller.runAjaxOpenDiagram(controller, DBDesigner._PConfig.getDefaultDiagram, projectID, null);
        } else {
            controller.runAjaxOpenDiagram(controller, DBDesigner._PConfig.openDiagram, projectID, diagramID);
        }
    },
